...
module.exports = function(data){
	...
	if(parseInt(res[0])===20 && parseInt(res[1])!==0){
		...
		if(param.device.protocol !== 'Debug' && param.device.protocol !== 'Slave'){
			...
			return gladys.deviceState.createByIdentifier(param.device.identifier, param.device.service, param.types[0].type, statelist.states[0])
				.catch(() => {
				...
				.then(function(result) {
					for (var i = 0, len = nbtype; i < len; i++) {
						// we create the state
						return gladys.deviceState.createByIdentifier(param.device.identifier, param.device.service, param.types[i].type, statelist.states[i])
					}
				});
		}
	}
