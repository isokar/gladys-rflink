			...
			for (var i = 4, len = maxlength; i < len; i++) {
				...
				switch(item[0]){
					...
					case `RGBWW`:
						...
						res.push('BRIGHT=' + txtval.substring(2, 3));
						//res.push('BRIGHTNESS='+txtval.substring(2,4));old value
						tmpmin = 0;
						tmpmax = 255;
						len++;
				...
				if(update){
					...
					param.types.push({state:'',type: 'binary',identifier: '',sensor: true,unit:'',min: tmpmin,max: tmpmax});
